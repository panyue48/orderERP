<template>
    ...
    <el-table :data="rows" style="width: 100%" :loading="loading">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="canExecute && row.status === 1" link type="success" @click="openPrecheck(row.id, row.billNo)">
            执行
          </el-button>
          <el-popconfirm
            v-if="canReverse && row.status === 2"
            :title="`确认冲销入库单 ${row.billNo} 吗？将生成一张冲销单并回滚库存。`"
  ...
  <el-dialog v-model="precheckVisible" title="执行前校验" width="900px">
    ...
    <template #footer>
      ...
      <el-button type="success" :loading="executingId === precheckBillId" :disabled="!precheck?.ok" @click="executePrechecked">
        确认执行
      </el-button>
    </template>
  </el-dialog>

  <el-dialog v-model="createVisible" title="新增盘点入库单" width="900px">
  ...
  <el-dialog v-model="detailVisible" title="盘点入库单详情" width="900px">
    <div v-if="detail">
      ...
      <el-table :data="detail.items" border>
        ...
        <el-table-column prop="productName" label="商品名称" min-width="220" />
        <el-table-column prop="unit" label="单位" width="80" />
        <el-table-column prop="qty" label="应收数量" width="140" />
        <el-table-column prop="realQty" label="实收数量" width="140" />
      </el-table>
    </div>
    <template #footer>
...
import { createStockInBill, executeStockInBill, getStockInBill, listProductOptions, listStockInBills, listWarehouseOptions, precheckStockInBill, reverseStockInBill, type ProductOption, type StockInBill, type StockInBillDetail, type WarehouseOption, type WmsBillPrecheckResponse } from '../api/wms'
import { useAuthStore } from '../stores/auth'

const auth = useAuthStore()
const canAdd = computed(() => auth.hasPerm('wms:stockin:add'))
const canExecute = computed(() => auth.hasPerm('wms:stockin:execute'))
const canReverse = computed(() => auth.hasPerm('wms:stockin:reverse'))
...
async function reload() {
  loading.value = true
  try {
    const res = await listStockInBills({ page: page.value, size: size.value, keyword: keyword.value || undefined })
...
async function submitCreate() {
  ...
  saving.value = true
  try {
    await createStockInBill({ warehouseId: createForm.value.warehouseId, remark: createForm.value.remark || undefined, lines })
...
async function openPrecheck(id: number, billNo: string) {
  ...
  try {
    precheck.value = await precheckStockInBill(id)
  } catch (e: any) {
...
async function reverse(id: number, billNo: string) {
  try {
    ...
    const res = await reverseStockInBill(id)
...
async function openDetail(id: number) {
  ...
  detail.value = null
  try {
    detail.value = await getStockInBill(id)
  } catch (e: any) {
