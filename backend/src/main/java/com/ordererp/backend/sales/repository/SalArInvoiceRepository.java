...
public interface SalArInvoiceRepository extends JpaRepository<SalArInvoice, Long> {
    @Query(value = """
            select
              i.id as id,
            ...
            from sal_ar_invoice i
            where i.bill_id = :billId
            order by i.id asc
            """, nativeQuery = true)
    List<SalArInvoiceRow> listRows(@Param("billId") Long billId);
